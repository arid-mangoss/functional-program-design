...
abstract class QuickCheckHeap2 extends Properties("Heap") with IntHeap {
  ...
  property(
    "Recusivly finding and deleting elements in a Heap should return same elements"
  ) = forAll { (h: H) =>
  ...
  property(
    "The minimal value of a melded Heap should be the min of the min of both heaps"
  ) = forAll { (h1: H, h2: H) =>
  ...
  property(
    "The minimal value of 2 heaps should be the minimal after dispacing it from heap 1 to 2 and melding both"
  ) = forAll { (h1: H, h2: H) =>
    val m1 = findMin(h1)
    val m2 = findMin(h2)
    val m = min(m1, m2)
